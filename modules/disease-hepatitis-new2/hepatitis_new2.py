...
URI_ROOT = 'urn:x-esphealth:disease:channing:hepatitis-new2:'
URI_VERSION = ':v1'
...
class Hepatitis_C(HepatitisNew):
    ...
    conditions = ['sandbox2:hepatitis_c:acute']

    uri = URI_ROOT + 'sandbox2:hepatitis_c:acute' + URI_VERSION

    short_name = 'sandbox2:hepatitis_c:acute'
    ...
    def _acute_hep_c_condition_a_b(self):
        ...
        for trigger_event in trigger_qs:
            ...
            created, this_case = self._create_case_from_event_obj(
                condition = 'sandbox2:hepatitis_c:acute', 
                criteria = criteria, 
            ...
            if created:
                counter += 1
        log.debug('Created %s new 2 Hep C cases with complex algo' % counter)
        return counter
    ...
    def _acute_hep_c_condition_c(self):
        ...
        Detects cases based on definitions (c) in spec for Hep C.
            c)hep c rna positive and record of hep c elisa negative within the prior 24 months
        ...
        for trigger_event in trigger_qs:
            ...
            relevancy_start_date = trigger_event.date - relativedelta(months=24)
            ...
            created, this_case = self._create_case_from_event_obj(
                condition = 'sandbox2:hepatitis_c:acute', 
                criteria = 'Criteria #3: positive c-rna and c-elisa negative w/in the prior 24 months',  
                recurrence_interval = None,  # Does not recur
                event_obj = trigger_event, 
            ...
            if created:
                counter += 1
        log.debug('Created %s new 2 Hep C cases condition c' % counter)
        return counter
    ...
    def _acute_hep_c_condition_d(self):
        ...
        Detects cases based on definitions (d) in spec for Hep C.    
            d)elisa positive and record of elisa negative within the prior 24 months AND no prior positive of elisa or riba or rna ever
        ...
        for trigger_event in trigger_qs:
            ...
            created, this_case = self._create_case_from_event_obj(
                condition = 'sandbox2:hepatitis_c:acute', 
                criteria = 'Criteria #4: c-elisa positive and c-elisa negative w/in the prior 24 months and no prior positive of c-elisa, c-riba or c-rna ever',  
                recurrence_interval = None,  # Does not recur
                event_obj = trigger_event, 
                relevant_event_qs = prior_neg_qs,
                )
            if created:
                counter += 1
        log.debug('Created %s new 2 Hep C cases condition d' % counter)
        return counter
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# Packaging
