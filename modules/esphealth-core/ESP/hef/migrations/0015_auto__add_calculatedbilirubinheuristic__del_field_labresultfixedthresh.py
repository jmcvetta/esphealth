...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'LabOrderHeuristic', fields ['test']
        db.delete_unique('hef_laborderheuristic', ['test_id'])

        # Removing unique constraint on 'LabResultAnyHeuristic', fields ['test']
        db.delete_unique('hef_labresultanyheuristic', ['test_id'])

        # Removing unique constraint on 'LabResultPositiveHeuristic', fields ['test']
        db.delete_unique('hef_labresultpositiveheuristic', ['test_id'])

        # Adding model 'CalculatedBilirubinHeuristic'
        db.create_table('hef_calculatedbilirubinheuristic', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('threshold', self.gf('django.db.models.fields.FloatField')()),
        ))
        db.send_create_signal('hef', ['CalculatedBilirubinHeuristic'])

        # Deleting field 'LabResultFixedThresholdHeuristic.name'
        db.delete_column('hef_labresultfixedthresholdheuristic', 'name')

        # Deleting field 'LabResultFixedThresholdHeuristic.notes'
        db.delete_column('hef_labresultfixedthresholdheuristic', 'notes')

        # Adding field 'Heuristic.notes'
        db.add_column('hef_heuristic', 'notes', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # Deleting field 'DiagnosisHeuristic.notes'
        db.delete_column('hef_diagnosisheuristic', 'notes')

        # Deleting field 'LabResultPositiveHeuristic.name'
        db.delete_column('hef_labresultpositiveheuristic', 'name')

        # Deleting field 'LabResultPositiveHeuristic.notes'
        db.delete_column('hef_labresultpositiveheuristic', 'notes')

        # Adding unique constraint on 'LabResultPositiveHeuristic', fields ['test', 'titer']
        db.create_unique('hef_labresultpositiveheuristic', ['test_id', 'titer'])

        # Deleting field 'LabResultRatioHeuristic.name'
        db.delete_column('hef_labresultratioheuristic', 'name')

        # Deleting field 'LabResultRatioHeuristic.notes'
        db.delete_column('hef_labresultratioheuristic', 'notes')

        # Deleting field 'LabResultAnyHeuristic.notes'
        db.delete_column('hef_labresultanyheuristic', 'notes')

        # Deleting field 'LabResultAnyHeuristic.name'
        db.delete_column('hef_labresultanyheuristic', 'name')

        # Adding unique constraint on 'LabResultAnyHeuristic', fields ['test']
        db.create_unique('hef_labresultanyheuristic', ['test_id'])

        # Deleting field 'LabOrderHeuristic.notes'
        db.delete_column('hef_laborderheuristic', 'notes')

        # Deleting field 'LabOrderHeuristic.name'
        db.delete_column('hef_laborderheuristic', 'name')

        # Adding unique constraint on 'LabOrderHeuristic', fields ['test']
        db.create_unique('hef_laborderheuristic', ['test_id'])
    ...
    def backwards(self, orm):
        ...
        # Removing unique constraint on 'LabResultPositiveHeuristic', fields ['test', 'titer']
        db.delete_unique('hef_labresultpositiveheuristic', ['test_id', 'titer'])

        # Deleting model 'CalculatedBilirubinHeuristic'
        db.delete_table('hef_calculatedbilirubinheuristic')

        # We cannot add back in field 'LabResultFixedThresholdHeuristic.name'
        raise RuntimeError(
            "Cannot reverse this migration. 'LabResultFixedThresholdHeuristic.name' and its values cannot be restored.")

        # Adding field 'LabResultFixedThresholdHeuristic.notes'
        db.add_column('hef_labresultfixedthresholdheuristic', 'notes', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # Deleting field 'Heuristic.notes'
        db.delete_column('hef_heuristic', 'notes')

        # Adding field 'DiagnosisHeuristic.notes'
        db.add_column('hef_diagnosisheuristic', 'notes', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # We cannot add back in field 'LabResultPositiveHeuristic.name'
        raise RuntimeError(
            "Cannot reverse this migration. 'LabResultPositiveHeuristic.name' and its values cannot be restored.")

        # Adding field 'LabResultPositiveHeuristic.notes'
        db.add_column('hef_labresultpositiveheuristic', 'notes', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # Adding unique constraint on 'LabResultPositiveHeuristic', fields ['test']
        db.create_unique('hef_labresultpositiveheuristic', ['test_id'])

        # We cannot add back in field 'LabResultRatioHeuristic.name'
        raise RuntimeError(
            "Cannot reverse this migration. 'LabResultRatioHeuristic.name' and its values cannot be restored.")

        # Adding field 'LabResultRatioHeuristic.notes'
        db.add_column('hef_labresultratioheuristic', 'notes', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # Adding field 'LabResultAnyHeuristic.notes'
        db.add_column('hef_labresultanyheuristic', 'notes', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # We cannot add back in field 'LabResultAnyHeuristic.name'
        raise RuntimeError(
            "Cannot reverse this migration. 'LabResultAnyHeuristic.name' and its values cannot be restored.")

        # Adding unique constraint on 'LabResultAnyHeuristic', fields ['test']
        db.create_unique('hef_labresultanyheuristic', ['test_id'])

        # Adding field 'LabOrderHeuristic.notes'
        db.add_column('hef_laborderheuristic', 'notes', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # We cannot add back in field 'LabOrderHeuristic.name'
        raise RuntimeError(
            "Cannot reverse this migration. 'LabOrderHeuristic.name' and its values cannot be restored.")
    ...
    models = {
        ...
        'hef.labresultpositiveheuristic': {
            'Meta': {'ordering': "['test']", 'unique_together': "(['test', 'titer'],)", 'object_name': 'LabResultPositiveHeuristic'},
            'date_field': ('django.db.models.fields.CharField', [], {'default': "'order'", 'max_length': '32'}),
            'heuristic_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['hef.Heuristic']", 'unique': 'True', 'primary_key': 'True'}),
            'test': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['hef.AbstractLabTest']"}),
