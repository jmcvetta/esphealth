...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for value in Event.objects.values_list('event_type', flat=True).distinct():
            substitute_value = 'legacy--%s' % value
            count = Event.objects.filter(event_type=value).count()
            log.debug("Updating %s '%s' events to '%s'" % (count, value, substitute_value))
            Event.objects.filter(event_type=value).update(event_type=substitute_value)
            et, created = EventType.objects.get_or_create(
                name = substitute_value,
                defaults = {'heuristic': legacy_heuristic,},
                )
            if created:
                log.debug('Created legacy event type for %s' % substitute_value)
