...
import os, django, sys, MySQLdb, time, datetime, copy, logging
...
espdb = MySQLdb.Connect('localhost', 'ESP', '3spuser2006')
...
class ICD9Events:
    ...
    def __iter__(self):
        ...
        #sql = '''select icd9,encId,demogId,encDate from esp.icd9Fact where demogId = %d and encDate >= '%s' and 
        #encDate <= '%s' order by encDate desc''' % (self.dId,self.firstDate,self.lastDate) # do we want latest first
        # we need to check exclusions all the way back
...
class PostImmEvents:
    ...
    def __iter__(self):
        ...
        try:
            ...
            logging.critical(s)
            raise StopIteration
        sql = """select * from esp.esp_immunization
          where round(ImmDate) >= %s and round(ImmDate) <= %s order by ImmPatient_id,ImmDate""" % (self.sps,self.spe)
        self.curs.execute(sql)
        res = self.curs.fetchall()
        # need multiple imms on any given day for case?
        idImmDate = {}
        logging.info( 'got %d imm records for period %s to %s' % (len(res),self.sps,self.spe))
        for nr in res: # create a dict in case subject has multiple imms on one date for vaers
            iId = nr[ID_KEY] # imm recid
            dId = int(nr[IMMPATIENTID_KEY]) # demogId
            iDate = nr[IMMDATE_KEY]
            iName = nr[IMMNAME_KEY]
            iCode = nr[IMMCODE_KEY]
            iBatch = nr[IMMLOT_KEY]
            iManuf = nr[IMMMANUF_KEY]
            iD = makeD(iDate)
            ...
            if currentdId <> dId: # need new demog - lookup
                ...
                if len(idrec) > 0:
                    ...
                    if sn.upper()[:2] == EXCLUDESURNAME[:2]:
                        fn = demog[DEMFN_KEY]
                        logging.info('#$$$$ patient id %s surname %s %s exluded - assumed to be a test case starting with %s' % (dId,
...
Sodium	<130	mmol/L	X > (LKV - 5)	Three"""
    ...
    lxmatch = [x[0].lower() for x in matchlist] # get names we need
    lxthresh = [x[1] for x in matchlist] # for eval later
    lxexcl = [x[3] for x in matchlist] # for eval later
    lxcat = dict(zip([x[0] for x in matchlist],[x[4] for x in matchlist])) # lookup
    lxlookup = [(x[1],x[3],x[0]) for x in matchlist]
    lxKeepdict = dict(zip(lxmatch,lxlookup)) # list of names we need to store - no point storing all
...
# 47.0 < float(x) < 47.1 which assumes that the decimals are in all icd9 codes
    ...
    for i,row in enumerate(matchlist): # deal with ranges and such for matches TODO do same for exclusions
        ...
        if len(icdkeeplist) > 1:
            ...
            newrow.append(checkSameLast12)
            keepme.append(newrow)
    #matchme = [matchme[x] for x in keepme] # remove multiple icd9 rows
    #matchme += addme # replace with expanded lists
    # also need to parse exclusion criteria

    icdkeepmatch = [x[1] for x in keepme] # get names we need
    
    # row has RuleTranslation  RuleICD9    RuleExcludes RuleCategory    Rulesource
