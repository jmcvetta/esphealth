...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        '''
        You may wish to execute the following SQL command before running this 
        migration.  It will purge all HEF Events that are not part of an 
        existing Nodis Case.  
        
            DELETE FROM hef_event 
            WHERE id NOT IN (
                SELECT DISTINCT event_id FROM nodis_case_events 
                UNION SELECT DISTINCT event_id FROM nodis_case_events_after 
                UNION SELECT DISTINCT event_id FROM nodis_case_events_before 
                UNION SELECT DISTINCT event_id FROM nodis_case_events_ever
            );
        '''
        ...
        db.add_column('hef_event', 'source', self.gf('django.db.models.fields.TextField')(default='legacy', db_index=True), keep_default=False)

        # Adding field 'Event.provider'
        db.add_column('hef_event', 'provider', self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['emr.Provider']), keep_default=False)

        # Adding field 'Event.note'
        db.add_column('hef_event', 'note', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # Adding unique constraint on 'Event', fields ['source', 'name', 'content_type', 'object_id']
        db.create_unique('hef_event', ['source', 'name', 'content_type_id', 'object_id'])

        # Deleting field 'Timespan.run'
        db.delete_column('hef_timespan', 'run_id')

        # Adding field 'Timespan.source'
        db.add_column('hef_timespan', 'source', self.gf('django.db.models.fields.TextField')(default='legacy', db_index=True), keep_default=False)

        # Changing field 'Timespan.pattern'
        db.alter_column('hef_timespan', 'pattern', self.gf('django.db.models.fields.TextField')(null=True))

        # Removing index on 'Timespan', fields ['pattern']
        db.delete_index('hef_timespan', ['pattern'])
