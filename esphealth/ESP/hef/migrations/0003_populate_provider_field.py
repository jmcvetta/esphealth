...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        Provider = orm['emr.Provider']
        counter = 0
        total = orm.Event.objects.count()
        for item in orm.Event.objects.all():
            content_type = ContentType.objects.get_for_id(item.content_type.pk)
            pro_pk = content_type.get_object_for_this_type(pk=item.object_id).provider.pk
            pro = Provider.objects.get(pk=pro_pk)
            item.provider = pro
            item.save()
            counter += 1
