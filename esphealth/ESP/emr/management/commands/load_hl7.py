...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#
# TODO: issue 330 This script must be re-factored to work as a manage.py command.
#
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
from ESP.static.models import Dx_codes
...
class Hl7MessageLoader(object):
    ...
    #TODO icd10  patched for icd9 for now
    def make_encounter(self):
        ...
        for dg1 in hl7.segments('DG1', self.message):
            ...
            try:
                ...
                dx_code_obj = Dx_codes.objects.get(code=dx_code_str, type='ICD9')
            except Dx_codes.DoesNotExist, e:
                log.warning('Unknown dx code: "%s".  Adding new code to table.' % dx_code_str)
                name = '[Generated by load_hl7.py - may not be valid.]'
                # TODO fix for icd10 patched for now
                dx_code_obj = Dx_codes(code=dx_code_str, name=name, type='ICD9')
                dx_code_obj.save()
            ...
            encounter.dx_codes.add(dx_code_obj)
            log.debug('Added dx code "%s" to %s' % (dx_code_obj, encounter))
        #
