...
class Hl7MessageLoader(object):
    ...
    def make_encounter(self):
        ...
        for dg1 in hl7.segments('DG1', self.message):
            ...
            dxcode_str = dg1[3][0]
            try:
                dxcode_obj = Dx_codes.objects.get(code=dxcode_str, type='ICD9')
            except Dx_codes.DoesNotExist, e:
                log.warning('Unknown dx code: "%s".  Adding new code to table.' % icd9_str)
                name = '[Generated by load_hl7.py - may not be valid.]'
                dxcode_obj = Dx_codes(code=dxcode_str, name=name,type='ICD9')
                dxcode_obj.save()
            ...
            encounter.dx_codes.add(dxcode_obj)
            log.debug('Added dx code "%s" to %s' % (dxcode_obj, encounter))
        #
