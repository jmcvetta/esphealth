-- Analysis script generated on Wednesday, 08 Jul 2015 13:35:51 EDT
-- Analysis name: opioidrx - HISTORICAL
-- Analysis description: Copy of opioidrx for historical creation
...
CREATE TABLE public.opi_a_100013_s_12  AS SELECT * FROM public.opi_a_100013_s_11 WHERE  type in (1,2);

-- Step 13: Derive Columns - OpioidRx, BenzodiaRx - Assign Condition Name, Criteria, Status, Notes, & Start Date in mdphnet format
CREATE TABLE public.opi_a_100013_s_13  AS SELECT T1.*,CASE when  (type) = 1  THEN  text('opioidrx')
...
CREATE TABLE public.opi_a_100013_s_16  AS SELECT T1.*,(((quantity_float*dosage_strength)*conversion_factor)*mod_refills) /  script_days  morphine_equiv FROM public.opi_a_100013_s_15 T1;
...
CREATE TABLE public.opi_a_100013_s_17  AS SELECT T1.patient_id,sum(script_days) total_days,sum( morphine_equiv) total_morph_equiv,min(T1.start_date) disease_date,T1.date_of_birth,T1.natural_key FROM  public.opi_a_100013_s_16 T1   GROUP BY T1.patient_id,T1.date_of_birth,T1.natural_key HAVING sum(morphine_equiv) >= 100 and sum(script_days) >=90;

-- Step 18: Derive Columns - HighOpioidDose - Assign Condition Name, Criteria, & Start Date in mdphnet format
CREATE TABLE public.opi_a_100013_s_18  AS SELECT T1.*,text('prescribed >= 100 milligrams of morphine equivalents per day for >= 90 days within the past year') criteria,text('highopioiduse') local_condition,disease_date  - ('1960-01-01'::date)  start_date_mdphnet FROM public.opi_a_100013_s_17 T1;

-- Step 19: Union - UNION the results together and exclude duplicates
CREATE TABLE public.opi_a_100013_s_19  AS (SELECT patient_id,natural_key,date_of_birth,disease_date,local_condition,local_criteria,start_date_mdphnet FROM public.opi_a_100013_s_10) UNION (SELECT patient_id,natural_key,date_of_birth,disease_date,local_condition,local_criteria,start_date_mdphnet FROM public.opi_a_100013_s_13) UNION (SELECT patient_id,natural_key,date_of_birth,disease_date,local_condition,criteria,start_date_mdphnet FROM public.opi_a_100013_s_18);

-- Step 20: Join - STANDARD - Find patients who don't already have this condition starting on this date.
CREATE TABLE public.opi_a_100013_s_20  AS SELECT T1.patient_id,T1.natural_key,T1.disease_date,T1.date_of_birth,T1.start_date_mdphnet,T1.local_condition,T1.local_criteria FROM public.opi_a_100013_s_19 T1 LEFT OUTER JOIN public.esp_condition T2 ON ((T1.natural_key = T2.patid) AND (T1.local_condition = T2.condition) AND (T1.start_date_mdphnet = T2.date))  WHERE (T2.patid is null);

-- Step 21: Derive Columns - STANDARD - local_notes, local_status, age_at_detect_year, centerid
CREATE TABLE public.opi_a_100013_s_21  AS SELECT T1.*,date_part('year', disease_date) - date_part('year', date_of_birth) age_at_detect_year,'1'::varchar(1) centerid,text('') local_notes,text('NO') local_status FROM public.opi_a_100013_s_20 T1;

-- Step 22: Derive Columns - STANDRD - age_group_10yr, age_group_5yr, age_group_ms
CREATE TABLE public.opi_a_100013_s_22  AS SELECT T1.*,CASE when (age_at_detect_year <= 9) then '0-9'
...
ELSE
'75+'
END  age_group_ms FROM public.opi_a_100013_s_21 T1;

-- Step 23: Subset Columns - STANDARD - Prepare Output
CREATE TABLE public.opi_a_100013_s_23  AS SELECT "centerid","natural_key","local_condition","start_date_mdphnet","age_at_detect_year","age_group_5yr","age_group_10yr","age_group_ms","local_criteria","local_status","local_notes" FROM public.opi_a_100013_s_22;

-- Step 24: Save - STANARD-- Populate ccda_opibenzo table
CREATE TABLE ccda_opibenzo  AS SELECT * FROM public.opi_a_100013_s_23;
