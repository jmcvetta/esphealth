...
create table esp_temp_smoking as
   select case when t1.latest='Yes' then 'Current'
           ...
           end as smoking::varchar(13), 
           t0.natural_key as patid
   from
     emr_patient t0
   left outer join 
     (select t00.tobacco_use latest, t00.patient_id 
   ...
   left outer join
     (select max(val) yesOrQuit, patient_id
   ...
   left outer join
     (select max(val) passive, patient_id
   ...
   left outer join
     (select max(val) never, patient_id
