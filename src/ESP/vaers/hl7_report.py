...
class onehl7:
    ...
    def buildOBX_data_1(self):
        ...
        age = self.demog._get_age(formatted=True)
        
        if len(age.split()) == 1:
            # Age string is one single number, no units specified,
            # Meaning age is represented in years.
            unit = getOnestr('^',['yr', 'year', 'ANSI'])
            value = age
        else:
            # Age string contains 'Months' in the end.
            unit = getOnestr('^',['mo', 'month', 'ANSI'])
            value = age.split()[0]


        temperature_str = self.case.encounter.EncTemperature if hasattr(self.case, 'encounter') else ''
...
def make_report(event):
    ...
    try:
        provider = event.patient.DemogProvider
    except:
        provider = None
    orc = testDoc.makeORC(provider) if provider else ''
    ...
    # Age
    age = testDoc.demog._get_age(formatted=True).split()
    
    try:
        if (len(age) == 1) and int(age[0])>5:
            pass
        else:
