...
from ESP.conf.models import NativeManufacturer, ImmunizationManufacturer
from ESP.utils.utils import log, Flexigrid

from forms import StandardVaccinesForm, StandardManufacturersForm
import datetime
...
def vaccine_detail(request, id):
    ...
    if is_post and form.is_valid():
        native = NativeVaccine.objects.get(code=id)
...
def manufacturer_detail(request, id):
    ...
    message = ''
    if is_post and form.is_valid():
        native = NativeManufacturer.objects.get(id=id)
