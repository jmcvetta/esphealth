...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        '''
        Find native_encounter_num values with more than one record, and for 
        each delete all but the most recently updated record.
        '''
        qs = orm.Encounter.objects.values('native_encounter_num')
        qs = qs.annotate(count=models.Count('pk'))
        qs = qs.filter(count__gte=2)
        native_encounter_numbers = qs.values('native_encounter_num')
        log_query('Duplicate encounters', native_encounter_numbers)
        for num in native_encounter_numbers:
            encs = orm.Encounter.objects.filter(native_encounter_num=num)
            log.debug('Native encounter number "%s" has %s records - removing all but most recent' % (num, encs.count()))
            most_recent_pk = encs.order_by('-updated_timestamp')[0].pk
            encs.exclude(pk=most_recent_pk).delete()
