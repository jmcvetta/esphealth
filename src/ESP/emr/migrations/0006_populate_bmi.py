...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        null_bmi_encs = orm.Encounter.objects.filter(bmi__isnull=True)
        total = null_bmi_encs.count()
        counter = 0
        for enc in null_bmi_encs:
            counter += 1
            enc._populate_bmi()
            print 'Populating BMI for Encounter %8s / %s' % (counter, total)
